...
class NameToIDMapper {
    ...
    public void saveMap(IDataAccessObject dataAccessObject, String database) throws TotalADSDBMSException {
        ...
        jsonObject.addProperty(NameToIDCollection.KEY.toString(), "nametoid");// we add a key here because it could //$NON-NLS-1$
    ...
    /**
     * Loads an existing map from a database, if exist
     *
     * @param dataAccessObject Data access object
     * @param database Model (or a database) name
     * @throws TotalADSDBMSException DBMS Exception
     */
    ...
    public void loadMap(IDataAccessObject dataAccessObject, String database) throws TotalADSDBMSException {
        ...
        try (IDBCursor cursor = dataAccessObject.selectAll(database,
            ...
            if (cursor.hasNext()) {
                ...
                if (obj != null) {
                    Type stringIntMap = new TypeToken<HashBiMap<Integer, String>>() { }.getType();
                    ...
                    // Transform id to name mapping to the name to id mapping
